...
## 🔜 งานถัดไปที่แนะนำ (เรียงตามความสำคัญ)

1. **สร้าง staff-safe view ก่อนรับพนักงานเข้าระบบ** — ดูหัวข้อ "ช่องว่างที่รู้แล้ว" ใน CLAUDE.md
   ตอนนี้ถ้าเชิญ staff เข้ามา จะใช้หน้าคลังสินค้าไม่ได้เลย
2. **Dead-man switch** (เช่น healthchecks.io) — สำคัญขึ้นมากหลังปิดแจ้งเตือนตี 3 ถ้า cron ตายจะเงียบสนิท
3. **เปิด Leaked Password Protection** ที่ Supabase Dashboard (1 คลิก)
4. **แตก `sc_opex` ออกจาก key-value store** — ต้นตอบั๊กเงินทุกตัวในเซสชันนี้ (งานใหญ่ ควรวางแผนแยก)
5. **เก็บกวาด `as any` ที่เหลือ 35 จุด + lint 110 error**
6. **ตรวจหน้าเว็บจริงด้วยตา** — เซสชันนี้แตะโค้ด 26 ไฟล์ ยืนยันด้วย typecheck/build/เทสต์/อ่าน DB จริง
   แต่ยังไม่ได้กดใช้งานจริงทุกหน้า โดยเฉพาะ `/inventory` (แก้จำนวนสต๊อก), `/adjustments`, `/pos`
