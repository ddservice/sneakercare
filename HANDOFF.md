...
**งานโค้ด:**
4. **แตก `sc_opex` ออกจาก key-value store** — ต้นตอบั๊กเงินทุกตัวที่เจอมา (งานใหญ่ ควรวางแผนแยก)
   หลักฐานล่าสุด: ส่วนแบ่งกำไรหุ้นส่วนถูกฝังอยู่ใน `misc_items_json` ปนกับค่าน้ำมันรถ จนต้องเขียน
   ตัวจับด้วย regex ใน `lib/expense-totals.ts` (2026-09-08)
5. **เพิ่มตัวเลือกเกณฑ์ "ตามบิล / เงินเข้าจริง"** ที่หน้าภาพรวม — ตอนนี้ล็อกเป็นเงินเข้าจริงอย่างเดียว
   เวลาเทียบกับ Excel ที่คิดตามบิลจะต่างกันโดยไม่มีอะไรอธิบายบนหน้าจอ
6. **เก็บกวาด `as any` ที่เหลือ ~35 จุด + lint 110 error**
7. **ตรวจหน้าเว็บจริงด้วยตา** โดยเฉพาะ `/inventory` (แก้จำนวนสต๊อก), `/adjustments`, `/pos`

**เก็บกวาด:**
8. **บัญชีทดสอบค้างใน `profiles`**: `stafftest_1788747294645` (role staff, id
   `6147e244-41b7-47b9-8f1a-c09795872339`) — เหลือจาก `npm run test:staff` ที่ลบไม่หมด
   เช็ค FK กับ `inv_audit_logs.performed_by` ก่อนลบเสมอ (ดูบทเรียนบัญชี `rlsverify35` ใน CLAUDE.md)
