## ✅ VAT จดทะเบียนเป็นระดับสาขา (2026-09-18)

- **แต่ละสาขาเป็นคนละนิติบุคคล** — `inv_branches.vat_registered` (default `true` ไม่เปลี่ยนสาขาเดิม) ไม่ใช่ `sc_settings` ทั้ง tenant · คีย์เก่า `sc_settings.vat_registered` เลิกเขียนแล้ว
- ตั้งค่าที่ `/settings` การ์ดสาขา (ช่อง “จด VAT”) · หน้าออกเอกสารอ่านสาขาที่เลือกที่หัวเว็บ (`lib/branch-vat.ts` `resolveBranchVat()`) · มีสาขาเดียวในกิจการใช้สาขานั้นอัตโนมัติ
- เลือก “ทุกสาขา” ทั้งที่มีหลายสาขา = ออกใบกำกับภาษีไม่ได้จนกว่าจะเลือกสาขา
- **ยังเป็นระดับกิจการ (ยังไม่แยกสาขา):** ชื่อร้าน / เลขผู้เสียภาษี / ที่อยู่ / PromptPay บนหัวบิล — อยู่ที่การ์ดร้านใน `/settings`
- **apply production แล้ว (2026-09-18):** `0043` ผ่าน SSH+psql — `inv_branches.vat_registered boolean not null default true` + refresh view `branches` (`security_invoker = on`) สาขาเดิม 2 แถวจด VAT ตาม default
- เทสต์: `npm run test:vat` · `scripts/test-migration-0043.mjs` · `npm run test:guards`
