...
function verifyToken(token) {
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    // cols: 0=username, 1=password, 2=role, 3=display_name, 4=full_name, 5=token, 6=token_expiry
    if (String(row[5]).trim() === String(token).trim()) {
      const expiry = new Date(row[6]);
      if (new Date() < expiry) {
        return { username: row[0], role: row[2], display_name: row[3], full_name: row[4], rowIndex: i + 1 };
      }
    }
  }
  return null;
...
function handleLogin(data) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const role  = String(row[2]).trim() || 'staff';
    const dname = String(row[3]).trim() || uname;
    const fname = String(row[4]).trim() || '';

    if (uname === username.trim() && pwd === String(password).trim()) {
