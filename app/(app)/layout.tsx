  ...
  return (
    <div id="app-shell" className="flex min-h-svh max-w-full flex-col overflow-x-clip bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 antialiased">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
        <div className="mx-auto flex h-14 max-w-7xl min-w-0 items-center gap-3 px-3 sm:px-6">
          <MobileNav
            items={mainNav}
            alerts={alerts}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
            ...
            <div className="hidden md:flex items-center gap-2 rounded-md border border-slate-200 bg-white py-0.5 pr-2.5 pl-0.5 dark:border-slate-700 dark:bg-slate-900">
              <span className="flex h-7 w-7 items-center justify-center rounded-md bg-teal-700 text-[11px] font-semibold text-white">
                {avatarLetter}
              </span>
              <span className="min-w-0 text-left">
                <span className="block max-w-36 truncate text-xs font-medium leading-4 text-slate-800 dark:text-slate-100">
            ...
            <Link
              ...
              className="flex h-8 w-8 items-center justify-center rounded-md text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 dark:text-slate-500 dark:hover:bg-slate-800 dark:hover:text-slate-200"
            >
            ...
            <form action={logout}>
              <Button
                ...
                variant="ghost"
                size="sm"
                className="h-8 w-8 p-0 rounded-md text-slate-400 dark:text-slate-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 hover:text-rose-500 dark:hover:text-rose-400 transition-colors"
                title="ออกจากระบบ"
              >
