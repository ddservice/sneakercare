...
**⚠️ ยังไม่ได้ทดสอบกับ production จริงแบบ end-to-end** (ต่างจากงานก่อนหน้าในเซสชันนี้ที่ทุกงานมี
`npm run test:multi-tenant` ยืนยันเสมอ) — เหตุผล: `fetchAllExpensesData()`/`fetchRosterStaff()`
เป็น Next.js Server Action ที่พึ่ง request context (cookies) เรียกจากสคริปต์ Node ธรรมดานอก
เฟรมเวิร์กไม่ได้ ต้องตรวจผ่านเบราว์เซอร์จริงหรือรอ deploy แล้วลองมือ — งานที่เหลือก่อนเชิญ LUXSU
เข้าระบบจริง: deploy แล้วตรวจ `/roster`+`/expenses` ด้วยเบราว์เซอร์จริงว่าไม่เห็นข้อมูลของ tenant #1
