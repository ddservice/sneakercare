...
- **Supplier (2026-07-12)**: `inv_suppliers` เป็น master data แยกต่างหาก ไม่ใช่แค่ช่องข้อความอิสระ — เลือกได้
  ...
  เหมือน `inv_items`
- **แก้ไขรายการซื้อเข้าที่กรอกผิด (2026-07-13)**: ปุ่ม "แก้ไข" ที่การ์ด "ประวัติการซื้อเข้า"
  **ไม่ได้ UPDATE แถวเดิมเด็ดขาด** (ผิดกฎ #2 append-only ledger) แต่ insert 2 แถวใหม่แทน: (1) รายการยกเลิก
  (quantity_delta ติดลบเท่ากับของเดิมเป๊ะ) (2) รายการที่แก้ไขแล้ว ทั้งคู่อ้าง `corrects_txn_id` กลับไปที่
  รายการต้นฉบับ — trigger คำนวณ moving-average ใหม่ให้เองอัตโนมัติจากทั้ง 2 แถว ไม่ต้องคำนวณเองฝั่ง JS
  แถวยกเลิก (`reference_type = 'correction'`) จะโชว์แบบจางๆ ในตารางและไม่มีปุ่มแก้ไขซ้ำ ถ้า insert แถวที่ 2
  (รายการใหม่) ล้มเหลวหลัง insert แถวที่ 1 (ยกเลิก) สำเร็จแล้ว จะมีข้อความเตือนให้แจ้ง Admin ตรวจสอบยอด
  (ไม่ได้ wrap เป็น DB transaction เดียวเพราะ insert ผ่าน supabase-js ธรรมดา 2 ครั้งแยกกัน)
- **ลบรายการซื้อเข้าที่กรอกซ้ำ/ผิด (2026-07-13)**: ปุ่ม "ลบ" ที่การ์ด "ประวัติการซื้อเข้า" (แยกจากปุ่ม
  "แก้ไข") ใช้ตอนต้องการเอารายการที่กรอกซ้ำ/ผิดออกทั้งหมด (ไม่ใช่แก้เป็นค่าใหม่) — insert รายการยกเลิกแถวเดียว
  (`quantity_delta` ติดลบเท่ากับของเดิมเป๊ะ, `reference_type = 'correction'`, อ้าง `corrects_txn_id`)
  ยอดคงเหลือ/มูลค่าจะถูกหักออกทันทีผ่าน trigger เดิม **บังคับกรอกเหตุผลก่อนกดยืนยันเสมอ** ไม่มีปุ่มลบไหน
  ในหน้าคลังสินค้าที่ไม่ต้องระบุเหตุผล
