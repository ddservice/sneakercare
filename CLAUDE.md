...
## ⬜ รอบถัดไปถ้าจะทำต่อ (2026-09-17) — ไม่บล็อกของที่เพิ่งขึ้น

งานเร่งที่เหลือไม่ใช่สูตรเงิน/RLS แล้ว ส่วนใหญ่เป็นชั้นแสดงผลและ CI:

1. **`npm run lint` แดงที่ `app/login/auth-gate.tsx`** — `setGate` ใน `useEffect`
   (`react-hooks/set-state-in-effect`) จากงานลิงก์เชิญ CI จึงแดงทุก push จนกว่าจะย้าย
   การตรวจ URL ออกจาก effect
2. **แถบเมนูหลักทั้งเว็บ** ยังเป็นพื้นเขียวทึบ — โมดูลคลังใช้เส้นใต้แล้ว แต่เมนูบนยังคนละยุค
3. **ตารางการเงิน** (`/expenses` `/dashboard` `/reports`) ยังหนาแบบ Excel — ใช้งานได้
   แต่ไม่สื่อความน่าเชื่อถือของระบบบัญชี
4. **หัวเว็บบนมือถือแน่น** — ตัวเลือกสาขา + ไอคอนหลายตัว ควรย้ายตัวเลือกสาขาเข้าเมนูแฮมเบอร์เกอร์
5. **healthchecks ของ CSV รายเดือน** ยังไม่มี check ตัวที่สอง (`HEALTHCHECK_CSV_URL`) —
   cron ตายจะรู้ช้าเป็นรายเดือน

จงใจไม่ทำในรอบคลัง เพื่อไม่ให้ปนกับบั๊กเลือกสาขาและการรีดีไซน์โมดูลคลัง
