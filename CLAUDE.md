...
**⚠️ ระเบิดเวลาที่ยังไม่ได้แก้:** แถว `sc_opex` category `payslip_detail` key `audit_log` เก็บ
...
**⚠️ พ.ค. 69 ยังไม่ตรง — รอเจ้าของตัดสิน** รายการ "ซื้อของเข้าร้าน" ในระบบกับ Excel ไม่ตรงกัน
ทั้งชื่อและยอด: ระบบมี `ซื้อของใช้เข้าร้านจาก Shoppee 1,228` + `ค่าอะไหล่เครื่องฉีดน้ำ 225`
+ `ตะกร้าผ้า อีเกีย 3 อัน 492` · Excel มี `ค่าซื้อของเข้าร้าน 1,496` + `ค่าซื้อของเข้าร้าน shoppee 5,405`
สุทธิแล้วระบบต่ำกว่า Excel **฿4,956.00** — ต้องรู้ก่อนว่ายอด Shoppee จริงคือ 1,228 หรือ 5,405
**ห้ามเดาแล้วเติมเอง** (บทเรียนจากเคส "ค่าที่ปรึกษา" ด้านล่าง)
...
- **production รัน commit `bd1bfba`** — `npm run deploy` สำเร็จ (การ์ด "กำไรก่อน/หลังแบ่ง"
  ขึ้นแล้ว) · ตรวจหลัง deploy: `/login` 200 · `/dashboard` 307 · `/expenses` 307 · PM2
  `sneakercare` (id 13) online
- คอมมิตหลังจากนั้นแก้แต่ข้อมูลใน DB กับเอกสาร ไม่ต้อง deploy ตาม
