...
- **Global Font = Prompt** (เปลี่ยนจาก IBM Plex Sans Thai เมื่อ 2026-09-08 ตามที่เจ้าของสั่ง)
  โหลดผ่าน `next/font/google` ใน `app/layout.tsx` → ตัวแปร `--font-prompt` → `--font-sans`/`--font-heading`
  ใน `app/globals.css` **ไม่มีที่ไหนในระบบ hardcode ชื่อฟอนต์อีก** เปลี่ยนฟอนต์ทั้งระบบแก้ 2 ไฟล์นี้พอ
  **⚠️ Prompt ไม่ใช่ variable font** ต้องระบุ `weight` ให้ครบทุกน้ำหนักที่ใช้ (300–700)
  ถ้าขาดตัวไหนเบราว์เซอร์จะปลอมน้ำหนักให้เอง (synthetic bold) แล้วตัวอักษรไทยจะดูเละ
...
**ที่ยังเหลือทำได้ฟรีถ้าอยากเร็วอีก:** `/pos/daily-entry` เรียก `countDailySales()` กับ
`fetchRecentDailySales()` แยกกันสองรอบ (รวมได้อีก ~230 ms)
