...
28. **สมุดซื้อหลาย connection** ต้อง unique ที่ตาราง ไม่ใช่แค่ JSON ใน `sc_settings` — จนกว่า `0046` จะ apply แอปใช้ CAS ค่าเดิม · ห้ามเรียก `sc_fn_post_receipt` / `sc_fn_guard_live_feature` บน production ก่อน apply
...
**`0046` อยู่ใน repo แล้วยังไม่ apply production:** ตาราง `sc_receipt_posts` + `sc_fn_post_receipt` + `sc_fn_guard_live_feature` · แอปยังลงสมุดผ่าน JSON ใน `sc_settings` (CAS) · ห้ามเรียก RPC ชุดนี้จาก production จนกว่าจะ apply
